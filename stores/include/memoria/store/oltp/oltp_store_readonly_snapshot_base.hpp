...
class OLTPStoreReadOnlySnapshotBase: public OLTPStoreSnapshotBase<Profile> {
protected:
...
public:
    OLTPStoreReadOnlySnapshotBase(
        SharedPtr<Store> store,
        CDescrPtr& snapshot_descriptor
    ):
        Base(store, snapshot_descriptor)
    {
        snapshot_descriptor_->ref();
    }

    ~OLTPStoreReadOnlySnapshotBase() noexcept {
        snapshot_descriptor_->unref();
    }
