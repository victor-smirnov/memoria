...
#include <memoria/reactor/count_down_latch.hpp>
...
public:
    ProcessBuilderImpl(boost::filesystem::path&& exe_path) :
...
class ProcessImpl {
    ...
    boost::fibers::count_down_latch<int32_t> process_status_latch_{1};
...
public:
    ProcessImpl(ProcessBuilderImpl* builder)
    {
        ...
        if (pid_ == -1)
        {
            MMA_THROW(SystemException()) << format_ex("Can't create process {}", builder->exe_path().string());
        }
        else if (pid_ == 0)
        {
            // child
