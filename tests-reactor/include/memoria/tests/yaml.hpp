...
struct IndirectStateFiledSerializer<AllocSharedPtr<IMemoryStore<T>>> {
    static void externalize(IMemoryStorePtr<T>& alloc, boost::filesystem::path path, ConfigurationContext* context)
    {
    ...
    static void internalize(IMemoryStorePtr<T>& alloc, boost::filesystem::path path, ConfigurationContext* context)
    {
        auto path_str = path.string();
        path_str += ".mma1";
