...
boost::filesystem::path get_tests_config_path()
{
...
boost::filesystem::path get_tests_output_path()
{
    auto& app = reactor::app();
...
TestStatus run_single_test(const U8String& test_path)
{
    ...
    if (test)
    {
        boost::filesystem::path output_dir_base = get_tests_output_path();
        boost::filesystem::path config_path     = get_tests_config_path();
        ...
        boost::filesystem::path test_output_dir = output_dir_base;
...
TestStatus replay_single_test(const U8String& test_path)
{
    ...
    if (test)
    {
        ...
        boost::filesystem::path config_path = test_output_dir;
    ...
    for (auto& suite: suites)
    {
        ...
        for (auto& test: suite.second->tests())
        {
            ...
            if (is_test_enabled(global_enabled, suite_enabled, test_enabled))
            {
                ...
                boost::filesystem::path std_output = test_output_dir;
                ...
                boost::filesystem::path std_error = test_output_dir;
...
void DefaultTestContext::failed(TestStatus detail, std::exception_ptr ex, TestState* state) noexcept
{
    ...
    if (detail == TestStatus::TEST_FAILED && !state->is_replay())
    {
        ...
        try {
            boost::filesystem::path config_path = this->data_directory();
            config_path.append("config.yaml");
...
std::shared_ptr<WorkerProcess> MultiProcessRunner::create_worker(size_t num)
{
    ...
    boost::filesystem::create_directories(output_dir_base);
...
void MultiProcessRunner::handle_connections()
{
    ...
    {
        boost::fibers::fiber ff = reactor::in_fiber([&](SocketConnectionData&& conn_data) {
...
boost::fibers::fiber WorkerProcess::start()
{
    boost::filesystem::path output_dir_base(output_folder_);
    ...
    boost::filesystem::path std_output = output_dir_base;
    std_output.append("stdout.txt");
    out_file_ = open_buffered_file(std_output, FileFlags::CREATE | FileFlags::RDWR | FileFlags::TRUNCATE);

    boost::filesystem::path std_error = output_dir_base;
    std_error.append("stderr.txt");
