...
void register_type_reflection_256(TypeReflection& type_reflection) {
    ...
    auto datatype = hermes::HermesCtrView::parse_datatype(type_reflection.str()).root().value().as_datatype();
...
U8String normalize_type_declaration(U8StringView type_decl) {
    return hermes::HermesCtrView::parse_datatype(type_decl).root().value().as_datatype().to_cxx_string();
}

UID256 compute_full_type_hash(U8StringView type_decl) {
    return hermes::HermesCtrView::parse_datatype(type_decl).root().value().as_datatype().cxx_type_hash();
}
