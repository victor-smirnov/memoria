...
{
public:
  typedef memoria::filesystem::path::value_type value_type;   // enables class path ctor taking directory_entry

  directory_entry() BOOST_NOEXCEPT {}
  explicit directory_entry(const memoria::filesystem::path& p) :
    m_path(p), m_status(file_status()), m_symlink_status(file_status())
  {
  }
  directory_entry(const memoria::filesystem::path& p,
  ...
  void assign(const memoria::filesystem::path& p,
  ...
  void replace_filename(const memoria::filesystem::path& p,
    file_status st = file_status(), file_status symlink_st = file_status())
  {
...
# ifndef MEMORIA_BOOST_FILESYSTEM_NO_DEPRECATED
  void replace_leaf(const memoria::filesystem::path& p, file_status st, file_status symlink_st)
  {
    replace_filename(p, st, symlink_st);
  }
# endif

  const memoria::filesystem::path& path() const BOOST_NOEXCEPT { return m_path; }
  operator const memoria::filesystem::path&() const BOOST_NOEXCEPT { return m_path; }
...
private:
  memoria::filesystem::path   m_path;
...
//  - a conversion to 'path' using 'operator const memoria::filesystem::path&()',
...
template<>
struct range_mutable_iterator<memoria::filesystem::directory_iterator, void>
{
  typedef memoria::filesystem::directory_iterator type;
};
...
template<>
struct range_const_iterator<memoria::filesystem::directory_iterator, void>
{
  typedef memoria::filesystem::directory_iterator type;
};
...
//  namespace memoria::v1 template specializations
template<>
struct range_mutable_iterator<memoria::filesystem::recursive_directory_iterator, void>
{
  typedef memoria::filesystem::recursive_directory_iterator type;
};
template<>
struct range_const_iterator<memoria::filesystem::recursive_directory_iterator, void>
{
  typedef memoria::filesystem::recursive_directory_iterator type;
};
