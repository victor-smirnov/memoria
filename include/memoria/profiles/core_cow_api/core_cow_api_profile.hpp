...
struct ApiProfileTraits<CoreCowApiProfile<TT>> {
    using SnapshotID    = UUID;
    using CtrID         = UUID;
    using CtrSizeT      = int64_t;
    using ApiProfileT   = CoreCowApiProfile<TT>;
...
template <typename TT>
struct CtrReferenceable<CoreCowApiProfile<TT>>: CtrReferenceableBase<CoreCowApiProfile<TT>> {
    using ApiProfile = CoreCowApiProfile<TT>;
