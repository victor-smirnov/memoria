...
MEMORIA_CONTAINER_PART_BEGIN(memoria::metamap::CtrNavName)
    ...
    Iterator End()
    {
        ...
        typename Types::template SkipForwardWalker<Types, IntList<0>> walker(stream, 0, self.size());
    ...
    Iterator RBegin()
    {
        ...
        if (size > 0)
        {
            typename Types::template SkipForwardWalker<Types, IntList<0>> walker(stream, 0, size - 1);
            return self.find0(stream, walker);
        }
        else {
            return self.End();
