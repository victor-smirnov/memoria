...
public:
    ...
    Int32Result ctr_get_branch_node_capacity(const TreeNodeConstPtr& node, uint64_t active_streams) const noexcept
    {
    ...
    VoidResult ctr_split_branch_node(const TreeNodePtr& src, const TreeNodePtr& tgt, int32_t split_at) noexcept;
...
M_PARAMS
VoidResult M_TYPE::ctr_split_branch_node(const TreeNodePtr& src, const TreeNodePtr& tgt, int32_t split_at) noexcept
{
    auto& self = this->self();
...
VoidResult M_TYPE::ctr_create_new_root_block(TreePathT& path) noexcept
{
    ...
    MEMORIA_TRY_VOID(self.ctr_root_to_node(root.as_mutable()));
