...
#include <memoria/core/packed/tree/fse_max/packed_fse_max_tree_so.hpp>
...
//    using IOSubstreamView     = io::IOColumnwiseFixedSizeArraySubstreamViewImpl<Value, Blocks>;
    ...
    VoidResult init(int32_t capacity = 0)
    {
        MEMORIA_TRY_VOID(Base::init(block_size(capacity), Blocks * SegmentsPerBlock + 1));
