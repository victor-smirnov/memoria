...
public:
    ...
    static constexpr size_t OffsetsBlock = Block_ + 1;
    ...
    size_t* offsets() {
        return pkd_buf_->template get<size_t>(OffsetsBlock);
    }
    ...
    {
        MEMORIA_TRY(offsets, alloc->template allocate_array_by_size<size_t>(OffsetsBlock, 1));
    ...
    VoidResult insert_space(size_t start, size_t extra_size, size_t extra_data_length) noexcept
    {
        auto& meta = pkd_buf_->metadata();
        ...
        MEMORIA_TRY_VOID(pkd_buf_->resize_block(OffsetsBlock, offsets_length));

        size_t column_data_length = extra_data_length + meta.data_size(Dimension);
    ...
    VoidResult remove_space(size_t start, size_t size) noexcept
    {
        ...
        MEMORIA_TRY_VOID(pkd_buf_->resize_block(OffsetsBlock, new_offsets_length));

        size_t column_data_length = meta.data_size(Dimension) * sizeof(T);
    ...
    VoidResult resize_row(size_t idx, const Span<const T>& value)
    {
        ...
        if (current_value_size < new_value_size)
        {
            ...
            size_t column_data_length = size_delta + meta.data_size(Dimension);
        ...
        else if (current_value_size > new_value_size)
        {
            ...
            size_t column_data_length = meta.data_size(Dimension) - size_delta;
    ...
    VoidResult replace_row(size_t idx, const Span<const T>& value) noexcept
    {
        auto* data = this->data();
    ...
    {
        size_t column_data_size = meta.data_size(Dimension);
        size_t view_length = span.length();

        size_t offsets_size = meta.offsets_size();

        size_t offsets_block_size_aligned0 = pkd_buf_->element_size(OffsetsBlock);
    ...
    size_t estimate_replace_upsize(size_t idx, const Span<const T>& span, const Metadata& meta) const
    {
        ...
        {
            size_t column_data_size = meta.data_size(Dimension);
            size_t column_data_size_aligned0 = pkd_buf_->element_size(OffsetsBlock);
    ...
    {
        FieldFactory<size_t>::serialize(buf, offsets(), meta.offsets_size());
    ...
    {
        FieldFactory<size_t>::deserialize(buf, offsets(), meta.offsets_size());
