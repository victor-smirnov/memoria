...
class PackedFixedElementOptArraySO {
    ...
    using MyType = PackedFixedElementOptArraySO;
...
public:
    ...
    using ArrayValues   = typename PkdStruct::Array::Values;
    ...
    PackedFixedElementOptArraySO():
        data_(), array_()
    {}

    PackedFixedElementOptArraySO(const ExtData* ext_data, PkdStruct* data):
...
//    }
    ...
    template <typename T>
    VoidResult insert(int32_t idx, const core::StaticVector<T, Columns>& values)
    {
        Bitmap* bitmap = data_->bitmap();
    ...
    VoidResult insert(int32_t idx, int32_t size, Fn&& provider, bool reindex = true) noexcept
    {
        auto bitmap = data_->bitmap();
        int32_t bitidx = 0;
        int32_t setted = 0;
        ...
        MEMORIA_TRY_VOID(array_.insert(array_idx, setted, [&, this](int32_t) -> const auto& {
            while(true)
            {
                if (tidx < size)
                {
