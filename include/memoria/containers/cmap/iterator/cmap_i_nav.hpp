...
#ifndef _MEMORIA_CONTAINERS_CMAP_ITER_NAV_HPP
#define _MEMORIA_CONTAINERS_CMAP_ITER_NAV_HPP
...
MEMORIA_ITERATOR_PART_BEGIN(memoria::cmap::ItrNavName)
...
#define M_TYPE      MEMORIA_ITERATOR_TYPE(memoria::cmap::ItrNavName)
...
bool M_TYPE::nextLeaf()
{
    ...
    auto next = self.model().getNextNodeP(self.leaf());
...
bool M_TYPE::prevLeaf()
{
    ...
    auto prev = self.model().getPrevNodeP(self.leaf());
...
bool M_TYPE::hasNextKey()
{
	...
	if (!self.isEnd())
    {
        if (self.idx() < ctr.getNodeSize(self.page(), 0) - 1)
        {
            return true;
        }
        else {
