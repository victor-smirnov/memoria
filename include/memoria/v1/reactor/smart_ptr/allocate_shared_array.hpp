...
*/
#ifndef MMA1_SMART_PTR_ALLOCATE_SHARED_ARRAY_HPP
#define MMA1_SMART_PTR_ALLOCATE_SHARED_ARRAY_HPP
...
struct sp_if_size_array<T[N]> {
    typedef reactor::shared_ptr<T[N]> type;
};
...
sp_array_destroy(A&, T*, std::size_t) noexcept { }
...
public:
    ...
    template<class U>
    sp_array_state(const U& allocator, std::size_t size) noexcept
...
public:
    ...
    template<class U>
    sp_size_array_state(const U& allocator, std::size_t) noexcept
        : allocator_(allocator) { }
...
template<class T, class U>
inline U*
sp_array_start(void* base) noexcept
{
    enum {
...
public:
    ...
    virtual void* get_local_deleter(const boost::detail::sp_typeinfo&) {
        return 0;
    }
