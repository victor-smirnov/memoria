...
template<typename T> reactor::shared_ptr<T> shared_from_raw(T *);
template<typename T> reactor::weak_ptr<T> weak_from_raw(T *);
...
private:
    ...
    template<class X, class Y> void _internal_accept_owner( shared_ptr<X> * ppx, Y * ) const
    {
        ...
        else if( shared_this_.use_count() != 0 )
        {
            ...
            detail::esft2_deleter_wrapper * pd = reactor::get_deleter<detail::esft2_deleter_wrapper>( shared_this_ );
            BOOST_ASSERT( pd != 0 );
...
template<typename T>
reactor::shared_ptr<T> shared_from_raw(T *p)
{
    BOOST_ASSERT(p != 0);
    return reactor::shared_ptr<T>(p->enable_shared_from_raw::shared_from_this(), p);
}

template<typename T>
reactor::weak_ptr<T> weak_from_raw(T *p)
{
    BOOST_ASSERT(p != 0);
    reactor::weak_ptr<T> result;
