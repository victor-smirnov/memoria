...
template <typename Types>
struct PkdStructSizeType<PkdFMOTree<Types>> {
    static const PackedSizeType Value = PackedSizeType::FIXED;
};


template <typename Types>
struct StructSizeProvider<PkdFMOTree<Types>> {
    static const int32_t Value = PkdFMOTree<Types>::Blocks;
};

template <typename Types>
struct IndexesSize<PkdFMOTree<Types>> {
    static const int32_t Value = PkdFMOTree<Types>::Blocks;
};

template <typename T>
struct PkdSearchKeyTypeProvider<PkdFMOTree<T>> {
    using Type = typename PkdFMOTree<T>::Value;
};
