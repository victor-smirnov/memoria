...
namespace mapped_ {
    ...
    struct ObjectCreator<T, false> {
        ...
        static auto allocate_and_construct(Arena* arena, CtrArgs&&... args)
        {
...
public:
    ...
    MMA1_NODISCARD virtual size_t allocate_space(size_t size, size_t alignment, size_t tag_size = 0) noexcept = 0;
    ...
    template <typename T>
    MMA1_NODISCARD PtrT<T> allocate_space(size_t size) noexcept
    {
        size_t alignment = alignof(T);
        return allocate_space(size, alignment);
    }
...
class MappedArena: public MappedArenaBase<uint64_t> {
    using Base = MappedArenaBase<uint64_t>;

    ArenaBuffer<uint8_t> arena_;
public:
    using ArenaBase = Base;

    MappedArena() noexcept : arena_(64, [&, this](int32_t buffer_id, ArenaBufferCmd cmd, size_t size, uint8_t*) -> uint8_t* {
    ...
    virtual size_t allocate_space(size_t size, size_t alignment, size_t tag_size) noexcept
    {
