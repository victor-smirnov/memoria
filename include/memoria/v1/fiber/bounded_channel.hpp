...
private:
    ...
    void close_( std::unique_lock< memoria::v1::fibers::mutex > & lk) noexcept {
    ...
    channel_op_status push_( ptr_t new_node,
                             std::unique_lock< memoria::v1::fibers::mutex > & lk) {
        if ( is_closed_() ) {
    ...
    channel_op_status try_push_( ptr_t new_node,
                                 std::unique_lock< memoria::v1::fibers::mutex > & lk) noexcept {
        if ( is_closed_() ) {
            return channel_op_status::closed;
        }
        if ( is_full_() ) {
    ...
    value_type value_pop_( std::unique_lock< memoria::v1::fibers::mutex > & lk) {
