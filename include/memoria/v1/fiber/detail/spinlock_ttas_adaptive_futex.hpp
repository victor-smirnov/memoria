...
public:
    ...
    void lock() noexcept {
        ...
        const std::int32_t max_tests = (std::min)( static_cast< std::int32_t >( MEMORIA_V1_FIBERS_SPIN_MAX_TESTS), 2 * prev_tests + 10);
        // after max. spins or collisions suspend via futex
        while ( max_tests > tests && MEMORIA_V1_FIBERS_SPIN_MAX_COLLISIONS > collisions) {
