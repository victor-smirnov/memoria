...
public:
    ...
    Position leafrank_(const NodeBaseG& leaf, Position sizes, const LeafPrefixRanks& prefixes, Int pos, Args&&... args) const
    {
        ...
        else
        {
            for (Int s = 0; s < Streams; s++)
            {
                ...
                if (pos >= sum)
                {
                    return btfl::detail::ZeroRankHelper<0, Streams>::process(this, leaf, sizes, prefixes[s], pos, std::forward<Args>(args)...);
                }
                else {
                    sizes[s] = 0;
                }
    ...
    void build_node_layout(const NodeBaseG& leaf, const Position& extent, Fn&& fn) const
    {
        ...
        btfl::iobuf::FlatTreeStructureBuilder<MyType, Fn> builder(self, fn);
...
protected:
    ...
    struct _StreamsRankFn {
        ...
        {
            btfl::detail::StreamsRankFn<
                LeafSizesSubstreamPath,
                CtrSizeT,
                Position
            > fn(sizes, prefix, pos, anchors, anchor_values);

            btfl::detail::StreamsRankHelper<Stream, SearchableStreams>::process(leaf, fn, std::forward<Args>(args)...);
