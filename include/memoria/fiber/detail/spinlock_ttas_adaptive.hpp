...
#ifndef MEMORIA_FIBERS_SPINLOCK_TTAS_ADAPTIVE_H
#define MEMORIA_FIBERS_SPINLOCK_TTAS_ADAPTIVE_H
    ...
    void lock() noexcept {
        ...
        for (;;) {
            ...
            const std::size_t max_relax_retries = (std::min)(
                    static_cast< std::size_t >( MEMORIA_FIBERS_SPIN_BEFORE_SLEEP0), 2 * prev_retries + 10);
            const std::size_t max_sleep_retries = (std::min)(
                    static_cast< std::size_t >( MEMORIA_FIBERS_SPIN_BEFORE_YIELD), 2 * prev_retries + 10);
...
#endif // MEMORIA_FIBERS_SPINLOCK_TTAS_ADAPTIVE_H
