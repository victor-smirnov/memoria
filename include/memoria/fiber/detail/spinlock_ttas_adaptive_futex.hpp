...
#ifndef MEMORIA_FIBERS_SPINLOCK_TTAS_ADAPTIVE_FUTEX_H
#define MEMORIA_FIBERS_SPINLOCK_TTAS_ADAPTIVE_FUTEX_H
...
#include <memoria/fiber/detail/cpu_relax.hpp>
#include <memoria/fiber/detail/futex.hpp>
...
public:
    ...
    void lock() noexcept {
        ...
        const std::int32_t max_relax_retries = (std::min)(
                static_cast< std::int32_t >( MEMORIA_FIBERS_SPIN_BEFORE_SLEEP0), 2 * prev_retries + 10);
        const std::int32_t max_sleep_retries = (std::min)(
                static_cast< std::int32_t >( MEMORIA_FIBERS_SPIN_BEFORE_YIELD), 2 * prev_retries + 10);
        // after max. spins or collisions suspend via futex
        while ( retries++ < MEMORIA_FIBERS_RETRY_THRESHOLD) {
            ...
            if ( 0 != ( expected = value_.load( std::memory_order_relaxed) ) ) {
                ...
                std::uniform_int_distribution< std::int32_t > distribution{
                    0, static_cast< std::int32_t >( 1) << (std::min)(collisions, static_cast< std::int32_t >( MEMORIA_FIBERS_CONTENTION_WINDOW_THRESHOLD)) };
...
#endif // MEMORIA_FIBERS_SPINLOCK_TTAS_ADAPTIVE_FUTEX_H
