...
#ifndef MEMORIA_FIBERS_SPINLOCK_RTM_H
#define MEMORIA_FIBERS_SPINLOCK_RTM_H
...
#include <memoria/fiber/detail/rtm.hpp>
#include <memoria/fiber/detail/spinlock_status.hpp>
...
public:
    ...
    void lock() noexcept {
        ...
        for ( std::size_t retries = 0; retries < MEMORIA_FIBERS_RETRY_THRESHOLD; ++retries) {
            std::uint32_t status;
            ...
            if ( rtm_status::none != (status & rtm_status::may_retry) ||
                ...
                if ( MEMORIA_FIBERS_CONTENTION_WINDOW_THRESHOLD > collisions) {
                ...
                while ( spinlock_status::locked == splk_.state_.load( std::memory_order_relaxed) ) {
                    if ( MEMORIA_FIBERS_SPIN_BEFORE_SLEEP0 > count) {
                        ++count;
                        cpu_relax();
                    } else if ( MEMORIA_FIBERS_SPIN_BEFORE_YIELD > count) {
                        ++count; 
...
#endif // MEMORIA_FIBERS_SPINLOCK_RTM_H
