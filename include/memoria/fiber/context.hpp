...
#ifndef MEMORIA_FIBERS_CONTEXT_H
#define MEMORIA_FIBERS_CONTEXT_H
...
#include <memoria/core/tools/asan.hpp>
...
public:
    ...
    ::memoria::context::fiber suspend_with_cc() noexcept;
    ::memoria::context::fiber terminate() noexcept;
    ...
    friend void intrusive_ptr_release( context * ctx) noexcept {
        ...
        if ( 1 == ctx->use_count_.fetch_sub( 1, std::memory_order_release) ) {
            ...
            ::memoria::context::fiber c = std::move( ctx->c_);
            // destruct context
            ctx->~context();
            // deallocated stack
            std::move( c).resume();
...
#endif
#if defined(BOOST_NO_CXX17_STD_APPLY)
           ::memoria::context::detail::apply( std::move( fn), std::move( arg) );
#else
...
public:
    ...
    worker_context( launch policy,
                    ::memoria::context::preallocated const& palloc, StackAlloc && salloc,
                    Fn && fn, Arg ... arg
...
static ::boost::intrusive_ptr< context > make_worker_context( launch policy,
    ...
    return ::boost::intrusive_ptr< context >{
            new ( storage) context_t{
                policy,
                ::memoria::context::preallocated{ storage, size, sctx },
                std::forward< StackAlloc >( salloc),
                std::forward< Fn >( fn),
                std::forward< Arg >( arg) ... } };
}
