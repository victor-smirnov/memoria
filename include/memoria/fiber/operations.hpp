...
void sleep_until( std::chrono::time_point< Clock, Duration > const& sleep_time_) {
    std::chrono::steady_clock::time_point sleep_time = memoria::fibers::detail::convert( sleep_time_);
...
bool has_ready_fibers() noexcept {
    return memoria::fibers::context::active()->get_scheduler()->has_ready_fibers();
}

template< typename SchedAlgo, typename ... Args >
void use_scheduling_algorithm( Args && ... args) noexcept {
    memoria::fibers::context::active()->get_scheduler()
