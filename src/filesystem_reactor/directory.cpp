...
#define MEMORIA_BOOST_FILESYSTEM_USE_READDIR_R
#endif
...
//  MEMORIA_BOOST_FILESYSTEM_STATUS_CACHE enables file_status cache in
...
file_status directory_entry::get_status(boost::system::error_code* ec) const
{
...
MEMORIA_BOOST_FILESYSTEM_DECL
file_status directory_entry::get_symlink_status(boost::system::error_code* ec) const
{
...
#if defined(MEMORIA_BOOST_FILESYSTEM_USE_READDIR_R)
...
#endif // MEMORIA_BOOST_FILESYSTEM_USE_READDIR_R
...
#ifdef MEMORIA_BOOST_FILESYSTEM_STATUS_CACHE
...
MEMORIA_BOOST_FILESYSTEM_DECL
void directory_iterator_construct(directory_iterator& it, const path& p, unsigned int opts, boost::system::error_code* ec)
{
  if (error(p.empty() ? not_found_error_code : 0, p, ec,
            "memoria::filesystem::directory_iterator::construct"))
  {
    return;
  }
...
#     endif
    ...
    if (result)
    {
      if (result != make_error_condition(boost::system::errc::permission_denied) ||
        (opts & static_cast< unsigned int >(directory_options::skip_permission_denied)) == 0u)
      {
        error(result.value(), p,
          ec, "memoria::filesystem::directory_iterator::construct");
      }
...
MEMORIA_BOOST_FILESYSTEM_DECL
void directory_iterator_increment(directory_iterator& it, boost::system::error_code* ec)
{
  ...
  try
  {
    path::string_type filename;
    file_status file_stat, symlink_file_stat;
    boost::system::error_code increment_ec;
...
#       endif
      ...
      if (BOOST_UNLIKELY(!!increment_ec))  // happens if filesystem is corrupt, such as on a damaged optical disc
      {
        ...
        if (ec == NULL)
        {
          MEMORIA_BOOST_FILESYSTEM_THROW(
            filesystem_error("memoria::filesystem::directory_iterator::operator++",
              error_path,
              increment_ec));
        }
        *ec = increment_ec;
...
MEMORIA_BOOST_FILESYSTEM_DECL
void recursive_directory_iterator_construct(recursive_directory_iterator& it, const path& dir_path, unsigned int opts, boost::system::error_code* ec)
{
  if (ec)
    ec->clear();
...
MEMORIA_BOOST_FILESYSTEM_DECL
void recursive_directory_iterator_pop(recursive_directory_iterator& it, boost::system::error_code* ec)
{
  ...
  while (true)
  {
    ...
    if (BOOST_UNLIKELY(!!increment_ec))
    {
      ...
      {
        MEMORIA_BOOST_FILESYSTEM_THROW(
          filesystem_error("memoria::filesystem::recursive_directory_iterator::pop", increment_ec));
      }
...
inline push_directory_result recursive_directory_iterator_push_directory(detail::recur_dir_itr_imp* imp, boost::system::error_code& ec) BOOST_NOEXCEPT
{
  ...
  try
  {
    ...
    if ((imp->m_options & static_cast< unsigned int >(directory_options::follow_directory_symlink)) != 0u || !fs::is_symlink(symlink_stat))
    {
      ...
      {
        if (ec == make_error_condition(boost::system::errc::no_such_file_or_directory) && fs::is_symlink(symlink_stat) &&
...
MEMORIA_BOOST_FILESYSTEM_DECL
void recursive_directory_iterator_increment(recursive_directory_iterator& it, boost::system::error_code* ec)
{
