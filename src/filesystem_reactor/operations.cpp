...
boost::uintmax_t remove_all_aux_(const path& p, fs::file_type type,
  error_code* ec)
{
  ...
  if (type == fs::directory_file)  // but not a directory symlink
  {
    ...
    while(itr != end_dit)
    {
      ...
      count += remove_all_aux_(itr->path(), tmp_type, ec);
      if (ec != 0 && *ec)
        return count;
  ...
  error_code* ec)
{
    return mr::engine_or_local([&]() {
       return remove_all_aux_(p, type, ec);
    });
}
...
fs::file_status process_status_failure(const path& p, error_code* ec)
{
  ...
  if (ec == 0)
    MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error("memoria::filesystem::status",
...
path canonical(const path& p, const path& base, boost::system::error_code* ec)
{
  ...
  {
    if (ec == 0)
      MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error(
        "memoria::filesystem::canonical", source,
  ...
  else if (local_ec)
  {
    if (ec == 0)
      MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error(
        "memoria::filesystem::canonical", source, local_ec));
    *ec = local_ec;
    return result;
  }
...
void copy(const path& from, const path& to, boost::system::error_code* ec)
{
  ...
  {
    if (ec == 0)
      MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error("memoria::filesystem::copy",
...
# endif
  error(!BOOST_COPY_DIRECTORY(from.c_str(), to.c_str()) ? BOOST_ERRNO : 0,
    from, to, ec, "memoria::filesystem::copy_directory");
  });
}
...
void copy_file(const path& from, const path& to, copy_option option, error_code* ec)
{
  return mr::engine_or_local([&]() {
    error(!BOOST_COPY_FILE(from.c_str(), to.c_str(),
        ...
        from, to, ec, "memoria::filesystem::copy_file");
  });
}
...
# if defined(_WIN32_WINNT) && _WIN32_WINNT < 0x0600
  error(BOOST_ERROR_NOT_SUPPORTED, new_symlink, existing_symlink, ec,
    "memoria::filesystem::copy_symlink");
...
bool create_directory_(const path& p, error_code* ec)
{
  ...
  if (ec == 0)
    MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error("memoria::filesystem::create_directory",
      p, error_code(errval, system_category())));
  else
...
bool create_directories(const path& p, boost::system::error_code* ec)
{
 if (p.empty())
 {
   if (ec == 0)
     MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error(
       "memoria::filesystem::create_directories", p,
  ...
  if (!parent.empty())
  {
    ...
    if (parent_status.type() == file_not_found)
    {
      ...
      if (local_ec)
      {
        if (ec == 0)
          MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error(
            "memoria::filesystem::create_directories", parent, local_ec));
        else
          *ec = local_ec;
        return false;
      }
...
# if defined(BOOST_WINDOWS_API) && _WIN32_WINNT < 0x0600  // SDK earlier than Vista and Server 2008
  ...
  error(BOOST_ERROR_NOT_SUPPORTED, to, from, ec,
    "memoria::filesystem::create_directory_symlink");
# else
...
#   endif
  ...
  error(!BOOST_CREATE_SYMBOLIC_LINK(from.c_str(), to.c_str(),
    ...
    to, from, ec, "memoria::filesystem::create_directory_symlink");
# endif
  });
}
...
# if defined(BOOST_WINDOWS_API) && _WIN32_WINNT < 0x0500  // SDK earlier than Win 2K
  ...
  error(BOOST_ERROR_NOT_SUPPORTED, to, from, ec,
    "memoria::filesystem::create_hard_link");
# else
...
#   if defined(BOOST_WINDOWS_API) && _WIN32_WINNT >= 0x0600
      // see if actually supported by Windows runtime dll
      if (error(!create_symbolic_link_api ? BOOST_ERROR_NOT_SUPPORTED : 0, to, from, ec,
          "memoria::filesystem::create_symlink"))
        return;
#   endif

  error(!BOOST_CREATE_SYMBOLIC_LINK(from.c_str(), to.c_str(), 0) ? BOOST_ERRNO : 0,
    to, from, ec, "memoria::filesystem::create_symlink");
# endif
  });
}
...
#   if defined(__MSL__) && (defined(macintosh) || defined(__APPLE__) || defined(__APPLE_CC__))
          && err != 0
#   endif
        ) ? err : 0, ec, "memoria::filesystem::current_path");
    }
  };
  ...
  else if (BOOST_LIKELY(!local::getcwd_error(ec)))
  {
    for (std::size_t path_max = sizeof(small_buf);; path_max *= 2u) // loop 'til buffer large enough
    {
      ...
      {
        if (ec == 0)
          MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error("memoria::filesystem::current_path",
...
# else
  ...
  error(::GetCurrentDirectoryW(sz, buf.get()) == 0 ? BOOST_ERRNO : 0, ec,
    "memoria::filesystem::current_path");
  return path(buf.get());
# endif
  });
}
...
# else
  error(!BOOST_SET_CURRENT_DIRECTORY(p.c_str()) ? BOOST_ERRNO : 0,
    p, ec, "memoria::filesystem::current_path");
# endif
  });
}
...
# ifdef BOOST_POSIX_API
  ...
  if (e1 != 0 || e2 != 0)
  {
    ...
    error (e1 != 0 && e2 != 0, p1, p2, ec, "memoria::filesystem::equivalent");
    return false;
  }
...
# else  // Windows
  ...
  if (error(!::GetFileInformationByHandle(h1.handle, &info1) ? BOOST_ERRNO : 0,
    p1, p2, ec, "memoria::filesystem::equivalent"))
    return  false;
...
# ifdef BOOST_POSIX_API
  ...
  if (error(::stat(p.c_str(), &path_stat)!= 0 ? BOOST_ERRNO : 0,
      p, ec, "memoria::filesystem::file_size"))
...
# else  // Windows
  ...
  if (error(::GetFileAttributesExW(p.c_str(), ::GetFileExInfoStandard, &fad)== 0
    ? BOOST_ERRNO : 0, p, ec, "memoria::filesystem::file_size"))
        return static_cast<boost::uintmax_t>(-1);

  if (error((fad.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)!= 0
    ? ERROR_NOT_SUPPORTED : 0, p, ec, "memoria::filesystem::file_size"))
    return static_cast<boost::uintmax_t>(-1);
...
# ifdef BOOST_POSIX_API
  ...
  return error(::stat(p.c_str(), &path_stat)!= 0 ? BOOST_ERRNO : 0,
                p, ec, "memoria::filesystem::hard_link_count")
         ? 0
...
# ifdef BOOST_POSIX_API
  ...
  if (error(::stat(p.c_str(), &path_stat)!= 0,
      p, ec, "memoria::filesystem::is_empty"))
    return false;
...
# else
  ...
  WIN32_FILE_ATTRIBUTE_DATA fad;
  if (error(::GetFileAttributesExW(p.c_str(), ::GetFileExInfoStandard, &fad)== 0
    ? BOOST_ERRNO : 0, p, ec, "memoria::filesystem::is_empty"))
      return false;
...
# ifdef BOOST_POSIX_API
  ...
  struct stat path_stat;
  if (error(::stat(p.c_str(), &path_stat)!= 0 ? BOOST_ERRNO : 0,
    p, ec, "memoria::filesystem::last_write_time"))
      return std::time_t(-1);
...
#   if _POSIX_C_SOURCE >= 200809L
  ...
  {
    error(BOOST_ERRNO, p, ec, "memoria::filesystem::last_write_time");
    return;
  }
...
# ifdef BOOST_POSIX_API
  ...
  if (local_ec)
  {
    if (ec == 0)
    MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error(
        "memoria::filesystem::permissions", p, local_ec));
    else
      *ec = local_ec;
    return;
  }
...
# endif
  {
    ...
    if (ec == 0)
      MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error(
        "memoria::filesystem::permissions", p,
        error_code(err, boost::system::generic_category())));
    else
      ec->assign(err, boost::system::generic_category());
  }
...
# else  // Windows
  ...
  if (error(attr == 0 ? BOOST_ERRNO : 0, p, ec, "memoria::filesystem::permissions"))
    return;
  ...
  error(::SetFileAttributesW(p.c_str(), attr) == 0 ? BOOST_ERRNO : 0,
    p, ec, "memoria::filesystem::permissions");
# endif
  });
}
...
# ifdef BOOST_POSIX_API
  ...
  fail:
    const int err = errno;
    if (ec == 0)
      MEMORIA_BOOST_FILESYSTEM_THROW(filesystem_error("memoria::filesystem::read_symlink",
...
# elif _WIN32_WINNT < 0x0600  // SDK earlier than Vista and Server 2008
  error(BOOST_ERROR_NOT_SUPPORTED, p, ec,
        "memoria::filesystem::read_symlink");
# else  // Vista and Server 2008 SDK, or later
  ...
  if (error(h.handle == INVALID_HANDLE_VALUE ? BOOST_ERRNO : 0,
    p, ec, "memoria::filesystem::read_symlink"))
      return symlink_path;
...
path relative(const path& p, const path& base, error_code* ec)
{
  ...
  if (error(tmp_ec.value(), base, ec, "memoria::filesystem::relative"))
    return path();
...
# if defined(BOOST_POSIX_API)
  if (BOOST_UNLIKELY(size > static_cast< uintmax_t >((std::numeric_limits< off_t >::max)()))) {
    error(boost::system::errc::file_too_large, p, ec, "memoria::filesystem::resize_file");
    return;
  }
# endif
  error(!BOOST_RESIZE_FILE(p.c_str(), size) ? BOOST_ERRNO : 0, p, ec,
    "memoria::filesystem::resize_file");
  });
}
...
file_status status_(const path& p, error_code* ec)
{
# ifdef BOOST_POSIX_API
...
file_status symlink_status_(const path& p, error_code* ec)
{
# ifdef BOOST_POSIX_API
...
#   endif
  ...
  {
    error(ENOTDIR, p, ec, "memoria::filesystem::temp_directory_path");
    return p;
  }
...
# else  // Windows
  ...
  if (p.empty())
  {
    ...
    getwindir_error:
      int errval = ::GetLastError();
      error(errval, ec, "memoria::filesystem::temp_directory_path");
      return path();
    }
...
# else
  ...
  if (error(len == 0 ? BOOST_ERRNO : 0, p, ec, "memoria::filesystem::system_complete"))
    return path();
  ...
  return error(get_full_path_name(p, len , big_buf.get(), &pfn)== 0 ? BOOST_ERRNO : 0,
    p, ec, "memoria::filesystem::system_complete")
    ? path()
    : path(big_buf.get());
  });
# endif
...
path weakly_canonical(const path& p, boost::system::error_code* ec)
{
  ...
  for (; !head.empty(); --itr)
  {
    ...
    if (error(head_status.type() == fs::status_error,
      head, ec, "memoria::filesystem::weakly_canonical"))
      return path();
  ...
  if (error(tmp_ec.value(), head, ec, "memoria::filesystem::weakly_canonical"))
    return path();
  return tail.empty()
    ? head
