...
std::vector<FileChunk> create_chunks_vector(size_t max_chunk_size, uint64_t file_size)
{
...
auto file_block_test = register_test_in_suite<FnTest<FileBlockTestState>>(u"ReactorSuite", u"FileBlockTest", [](auto& state){
    ...
    for (uint64_t pos = 0; pos < total_written;)
    {
        size_t chunk_size = getRandomG(state.buffer_size - 1) + 1;

        if (pos + chunk_size >= total_written) {
            chunk_size = total_written - pos;
        }
