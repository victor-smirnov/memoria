...
public:
    ...
    using VectorMap = std::map<Key, std::vector<Value>>;
    ...
    MMA1_STATE_FILEDS(size, level_limit, last_level_limit, iterations)
    ...
    std::vector<Value> createValueData(size_t values, Fn2&& value_fn)
    {
    ...
    template <typename Fn2>
    std::vector<Key> createKeyData(size_t keys, Fn2&& key_fn)
    {
        std::vector<Key> data;
