...
public:
    ...
    PackedRLESearchableSequenceBufferTest()
    {
        size_ = 300;
    }
    ...
    {
        MMA1_CLASS_TESTS(suite, runInsertBufferRandom); //runCreateBuffer
    }
    ...
    void tryInsertBuffer(T1& seq, T2& buf, int32_t at, int32_t start, int32_t size, std::vector<T3>& seq_data, std::vector<T3>& buf_data)
    {
