...
#include <memoria/v1/fmt/posix.hpp>
...
memoria::v1::fmt::BufferedFile::~BufferedFile() noexcept {
    if (file_ && MMA1_FMT_SYSTEM(fclose(file_)) != 0)
        fmt::report_system_error(errno, "cannot close file");
}

memoria::v1::fmt::BufferedFile::BufferedFile(
...
memoria::v1::fmt::File::File(fmt::CStringRef path, int oflag) {
    int mode = S_IRUSR | S_IWUSR;
#if defined(_WIN32) && !defined(__MINGW32__)
    fd_ = -1;
    MMA1_FMT_POSIX_CALL(sopen_s(&fd_, path.c_str(), oflag, _SH_DENYNO, mode));
#else
...
memoria::v1::fmt::File::~File() noexcept {
    ...
    if (fd_ != -1 && MMA1_FMT_POSIX_CALL(close(fd_)) != 0)
        fmt::report_system_error(errno, "cannot close file");
}

void memoria::v1::fmt::File::close() {
    if (fd_ == -1)
        return;
    // Don't retry close in case of EINTR!
    // See http://linux.derkeiler.com/Mailing-Lists/Kernel/2005-09/3000.html
...
memoria::v1::fmt::LongLong memoria::v1::fmt::File::size() const {
    ...
    DWORD size_lower = MMA1_FMT_SYSTEM(GetFileSize(handle, &size_upper));
...
std::size_t memoria::v1::fmt::File::read(void *buffer, std::size_t count) {
    RWResult result = 0;
    MMA1_FMT_RETRY(result, MMA1_FMT_POSIX_CALL(read(fd_, buffer, convert_rwcount(count))));
    if (result < 0)
        throw SystemError(errno, "cannot read from file");
    return _::to_unsigned(result);
}

std::size_t memoria::v1::fmt::File::write(const void *buffer, std::size_t count) {
    RWResult result = 0;
...
memoria::v1::fmt::File memoria::v1::fmt::File::dup(int fd) {
...
void memoria::v1::fmt::File::dup2(int fd) {
    int result = 0;
...
void memoria::v1::fmt::File::dup2(int fd, ErrorCode &ec) noexcept {
    int result = 0;
    MMA1_FMT_RETRY(result, MMA1_FMT_POSIX_CALL(dup2(fd_, fd)));
    if (result == -1)
        ec = ErrorCode(errno);
...
void memoria::v1::fmt::File::pipe(File &read_end, File &write_end) {
    ...
    int result = MMA1_FMT_POSIX_CALL(pipe(fds, DEFAULT_CAPACITY, _O_BINARY));
#else
...
memoria::v1::fmt::BufferedFile memoria::v1::fmt::File::fdopen(const char *mode) {
...
long memoria::v1::fmt::getpagesize() {
#ifdef _WIN32
    SYSTEM_INFO si;
    GetSystemInfo(&si);
